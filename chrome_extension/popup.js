...
document.addEventListener('DOMContentLoaded', function() {
  ...
  reportBtn.addEventListener('click', function() {
    // Send report to content script and handle response
    chrome.tabs.sendMessage(currentTab.id, { action: 'reportSite' }, function(response) {
      if (chrome.runtime.lastError) {
        alert('Error: Could not connect to the page. Please make sure you are on an active webpage.');
      } else {
        // Close popup after successful report
        window.close();
      }
    });
  });
  ...
  clearBtn.addEventListener('click', function() {
    chrome.storage.local.set({ whitelistedDomains: [] }, function() {
      loadWhitelist([]);
      // Get current domain to update status
      chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
