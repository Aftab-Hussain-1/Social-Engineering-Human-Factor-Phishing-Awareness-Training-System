...
def download_extension():
    ...
    zip_filename = f'phishing_detector_{unique_id}.zip'
    zip_filepath = os.path.join(temp_dir, zip_filename)
    
    try:
        extension_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'chrome_extension')
        
        # Create zip file in memory instead of on disk
        memory_file = io.BytesIO()
        
        with zipfile.ZipFile(memory_file, 'w') as zipf:
            # Read and use the existing manifest.json
            ...
            if os.path.exists(manifest_path):
                ...
                raise FileNotFoundError('manifest.json not found in chrome_extension directory')

            # Add extension files from chrome_extension directory
            required_files = ['content.js', 'popup.html', 'popup.js', 'background.js', 'phishing_urls.json']
            for filename in required_files:
                ...
                if os.path.exists(file_path):
                    ...
                    # Use placeholder content for missing files
                    placeholder = f'// Placeholder for {filename}\n// This file should contain the implementation for {filename}'
                    zipf.writestr(filename, placeholder)

            # Add icon files
            icons_dir = os.path.join(extension_dir, 'icons')
            if os.path.exists(icons_dir):
                for icon_file in ['icon16.png', 'icon32.png', 'icon48.png', 'icon128.png']:
                    icon_path = os.path.join(icons_dir, icon_file)
                    if os.path.exists(icon_path):
                        with open(icon_path, 'rb') as f:
                            zipf.writestr(f'icons/{icon_file}', f.read())
                    else:
        ...
        # Return the in-memory zip file directly
        return send_file(
            memory_file,
            as_attachment=True,
            download_name='phishing_detector_extension.zip',
            mimetype='application/zip'
        )

    except FileNotFoundError as e:
        flash(f"Download failed: Required file not found - {str(e)}", "error")
...
def download_report_pdf(report_id):
    ...
    try:
        ...
        try:
            ...
            # Log the error and return HTML version as fallback
